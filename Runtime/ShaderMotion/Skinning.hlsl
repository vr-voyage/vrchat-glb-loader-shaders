...
void MorphVertex(inout Attributes i, Texture2D shapeTex) {
	uint2 size; shapeTex.GetDimensions(size.x, size.y);
	uint2 range = round(i.texcoord0.zw); // z=length, w=start so that the default (0,0,0,1) has length 0
...
void SkinVertex(inout Attributes i, Texture2D boneTex) {
	...
	for(uint J=0; J<maxBoneInfluence; J++) {
		...
		float4 data4[1];
		float3x3 mat = transpose(float3x3(i.vertex.xyz, i.normalOS.xyz, i.tangentOS.xyz));
